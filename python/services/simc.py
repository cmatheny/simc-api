...
import services.process_manager
from config import settings

class SimcService():

    def __init__(self):
        self.proc_man = services.process_manager.ProcessManager()
    ...
    def simc_armory_to_json(self, char_json):
        ...
        utils.file_manager.check_for_file(results_file)
        results_json = utils.file_manager.load_json(results_file)
        
        self.cleanup(results_file, job)
        return results_json

    def cleanup(self, results_file, job):
        utils.file_manager.remove_file(results_file)
